...
from backend.database.models import db, Student, Attendance, FaceEmbedding
...
def profile():
    ...
    total_attendance = Attendance.query.filter_by(student_id=student_id).count()
    total_sessions = max(total_attendance, 30)
    attendance_percentage = round((total_attendance / total_sessions) * 100, 1) if total_sessions > 0 else 0
